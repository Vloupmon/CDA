...
def test_sql_generate_model(modelname):
    ...
    import CommerceAPI.generate_model as gm
    import uuid
    import os.path
    ret = gm.generate_model(modelname, "sql", appname="CommerceAPI")
...
def test_sql_model_type():
    ...
    from CommerceAPI.models.sql.pow_test_model import PowTestModel
    m = PowTestModel()
...
def test_sql_dbsetup():
    ...
    import CommerceAPI.init_sqldb_environment
    import os
    os.chdir("..")
    r = CommerceAPI.init_sqldb_environment.init_migrations()
    assert r == True
...
def test_sql_migration():
    ...
    import CommerceAPI.generate_migration
    import os
    os.chdir("..")
    script = CommerceAPI.generate_migration.generate_migration(message="pow_test")
...
def test_sql_dbupdate():
    ...
    import CommerceAPI.update_db
    import os, time
    ret = None
    os.chdir("..")
    time.sleep(1)
    try:
        ret = CommerceAPI.update_db.migrate("up")
