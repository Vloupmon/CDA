...
namespace CommerceAPI_NetCore.Controllers {
    ...
    public class ProductsController : ControllerBase {
        ...
        // GET: api/Products/5/picture
        [HttpGet("{id}/picture")]
        public async Task<ActionResult<Picture>> GetPictureById(int id) {
            var picture = await (from pic in _context.Pictures
                                 join map in _context.ProductPictureMappings on pic.Id equals map.PictureId
                                 select map)
                                 .Where(x => x.ProductId == id && x.DisplayOrder == 1)
                                 .Select(s => s.Picture).FirstOrDefaultAsync();
